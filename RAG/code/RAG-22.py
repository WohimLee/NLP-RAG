...
def function1(input1,input2): #   知识库1-大模型
    global prompt_data,emb_database
    result1 = []
    result2 = []
    ...
    for p_n, group in prompt_data:
        ...
        for question in group["prompt"]:
            ...
            result1.append(f"检索及回答内容:\n{ans}")
            result1.append("*"*30)
            yield "\n".join(result1), "\n".join(result2)

        prompt_report = f"你是一个大学教授，你需要根据相关内容，来撰写一段内容，生成的结果必须严格来自相关内容，语言必须严谨、符合事实，不能使用第一人称，相关内容如下：\n```\n{''.join(p_n_content)}\n```\n生成的结果为："
        para_report = get_ans(prompt_report)
        para_report = para_report.replace("\n", "")

        all_report += "    " + para_report
        all_report += "\n"

        result1.append(f"第一段报告内容:\n{para_report}")
        result1.append("*"*30)
        result2.append(para_report)
